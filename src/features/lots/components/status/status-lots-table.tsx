...
export default function StatusLotsTable({ lots, isLoading, config }: Props) {
  ...
  const suppliers = useMemo(() => {
    if (!lots) return [];
    return [...new Set(lots.map((l) => l.supplier?.name).filter(Boolean))].sort() as string[];
  }, [lots]);
  ...
  const filteredLots = useMemo(() => {
    ...
    return lots.filter((lot) => {
      if (materialFilter !== "all" && lot.material_name !== materialFilter) return false;
      if (supplierFilter !== "all" && lot.supplier?.name !== supplierFilter) return false;
    ...
    <section className="flex min-h-[600px] flex-col overflow-hidden rounded-[32px] border border-emerald-100 bg-white">
      <div className="flex flex-col gap-5 px-8 py-7 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4">
          <h2 className="text-[24px] font-semibold leading-none text-zinc-900">Lots</h2>
        ...
        <div className="relative">
          <Button size="sm" leftIcon={ListFilter} onClick={() => setFilterOpen((prev) => !prev)}>
            Filter
          </Button>
          ...
          {filterOpen && (
            <div className="absolute right-0 top-12 z-50 w-[220px] max-h-[400px] overflow-y-auto rounded-2xl border border-zinc-200 bg-white shadow-xl">
              <div className="p-2">
                <p className="px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-zinc-400">Material</p>
                {[{ label: "All Materials", value: "all" }, ...materials.map((m) => ({ label: m, value: m }))].map((o) => (
                  <button key={o.value} type="button" onClick={() => { setMaterialFilter(o.value); setPage(1); }}
                ...
                <p className="mt-2 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-zinc-400">Supplier</p>
                {[{ label: "All Suppliers", value: "all" }, ...suppliers.map((s) => ({ label: s, value: s }))].map((o) => (
                  <button key={o.value} type="button" onClick={() => { setSupplierFilter(o.value); setPage(1); }}
                ...
                <p className="mt-2 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-zinc-400">Arrival Date</p>
                {[{ label: "All Time", value: "all" }, { label: "Today", value: "today" }, { label: "This Week", value: "week" }, { label: "This Month", value: "month" }].map((o) => (
                  <button key={o.value} type="button" onClick={() => { setDateFilter(o.value); setPage(1); }}
                ...
                <div className="px-3 py-2">
                  <input type="date" value={!["all", "today", "week", "month"].includes(dateFilter) ? dateFilter : ""}
              ...
              {isLoading ? (
                ...
                paginatedLots.map((lot, index) => (
                  <tr key={lot.id} className={`border-b border-zinc-100 ${index % 2 === 0 ? "bg-white" : "bg-zinc-50"}`}>
                    <td className="px-4 py-5 text-sm font-semibold text-zinc-800">{lot.lot_number}</td>
                    <td className="px-4 py-5 text-sm font-medium text-zinc-700">{lot.material_name}</td>
                    ...
                    <td className="px-4 py-5">
                      <Link href={`/lots/${lot.id}`} className="inline-flex items-center gap-1 text-sm font-semibold text-emerald-700 underline hover:text-emerald-800">
                        Details<ArrowUpRight className="size-4 translate-y-[1px]" />
                      </Link>
                    </td>
                  </tr>
                ))
