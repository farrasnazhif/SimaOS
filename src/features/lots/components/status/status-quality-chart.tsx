...
export default function StatusQualityChart({ lots }: Props) {
  ...
  const chartData = useMemo(() => {
    ...
    for (const lot of lots) {
      ...
      if (chartGradeFilter === "medium" && (score < 60 || score >= 80)) continue;
      if (chartGradeFilter === "low" && score >= 60) continue;
      const key = chartGroupBy === "material" ? lot.material_name : (lot.supplier?.name ?? "Unknown");
      const existing = grouped.get(key);
      if (existing) { existing.totalScore += score; existing.count += 1; }
      else { grouped.set(key, { totalScore: score, count: 1 }); }
    }
    return Array.from(grouped.entries()).map(([label, { totalScore, count }]) => ({
      label, avgScore: Math.round(totalScore / count), count,
    }));
  }, [lots, chartGroupBy, chartGradeFilter]);

  const lowest = chartData.length > 1 ? chartData.reduce((a, b) => (a.avgScore < b.avgScore ? a : b)) : null;
  const highest = chartData.length > 1 ? chartData.reduce((a, b) => (a.avgScore > b.avgScore ? a : b)) : null;
    ...
    <section className="col-span-12 overflow-hidden rounded-[32px] border border-emerald-100 bg-white lg:col-span-8">
      <div className="flex items-center justify-between px-8 py-7">
        <h2 className="text-[24px] font-semibold leading-none text-zinc-900">
          Quality Scores {chartGroupBy === "material" ? "by Material" : "by Supplier"}
        </h2>
        <div className="relative">
          <Button size="sm" leftIcon={ListFilter} onClick={() => setChartFilterOpen((prev) => !prev)}>
          ...
          {chartFilterOpen && (
            <div className="absolute right-0 top-12 z-50 w-[190px] max-h-[300px] overflow-y-auto rounded-2xl border border-zinc-200 bg-white shadow-xl">
              <div className="p-2">
                <p className="px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-zinc-400">Group By</p>
                {([{ label: "By Material", value: "material" }, { label: "By Supplier", value: "supplier" }] as const).map((o) => (
                  <button key={o.value} type="button" onClick={() => setChartGroupBy(o.value)}
                ...
                {[{ label: "All Grades", value: "all" }, { label: "High (80+)", value: "high" }, { label: "Medium (60–79)", value: "medium" }, { label: "Low (<60)", value: "low" }].map((o) => (
                  <button key={o.value} type="button" onClick={() => setChartGradeFilter(o.value)}
      ...
      <div className="px-6 pb-6">
        <div className="rounded-[18px] border border-emerald-100 bg-zinc-50 p-6">
          {chartData.length === 0 ? (
            <div className="flex h-[280px] items-center justify-center text-zinc-400">No data available.</div>
          ) : (
            <div className="flex gap-2" style={{ height: 280 }}>
              <div className="relative flex flex-1 items-end gap-4">
                <div className="pointer-events-none absolute inset-0 flex flex-col justify-between">
                  {[0, 1, 2, 3, 4].map((i) => (<div key={i} className="border-t border-zinc-200/60" />))}
                </div>
                {chartData.map((item) => (
                  <div key={item.label} className="group relative flex flex-1 flex-col items-center gap-2" style={{ height: "100%" }}>
                    <div className="relative flex w-full flex-1 items-end">
                      <div
                        className={`relative w-full min-h-[8px] rounded-t-2xl transition-all group-hover:opacity-80 ${
                          item.avgScore < 60 ? "bg-red-500" : item.avgScore < 75 ? "bg-amber-500" : "bg-emerald-600"
                        }`}
                    ...
                    <p className="text-center text-sm font-medium text-zinc-700">{item.label}</p>
                    <p className="text-xs text-zinc-400">{item.count} {item.count === 1 ? "lot" : "lots"}</p>
                  </div>
                ))}
              </div>
